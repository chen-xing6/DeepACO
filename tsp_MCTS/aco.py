    ...
    def search(self, root_state):
        ...
        for _ in range(self.n_simulations):
            ...
            while node.is_fully_expanded() and node.children and len(state) < self.n_nodes:
                ...
                state = node.state[:]
            # If fully expanded or terminal, simulate from here
            if len(state) == self.n_nodes:
                reward = self.simulate(state)  # but since complete, reward = -cost, but cost is for closed tour
                back_node = node
            else:
                # Expansion
